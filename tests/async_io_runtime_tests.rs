    ...
    let handles: Vec<_> = (0..4)
        .map(|_| {
            ...
            thread::spawn(move || {
                ...
                let h = handle().expect("runtime handle should be available");
...
fn test_run_blocking_does_not_create_new_runtimes() {
    ...
    let runtime_before = global().expect("runtime should initialize") as *const _;
    ...
    let runtime_after = global().expect("runtime should initialize") as *const _;
