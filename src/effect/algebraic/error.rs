...
//!
//! let result = ErrorHandler::new().run(computation).unwrap();
...
/// assert_eq!(handler.clone().run(ok_computation).unwrap(), Ok(42));
///
/// // Failed computation
/// let err_computation: Eff<ErrorEffect<String>, i32> =
///     ErrorEffect::throw("error".to_string());
/// assert_eq!(handler.run(err_computation), Err("error".to_string())).unwrap();
/// ```
...
impl<E: Clone + 'static> ErrorHandler<E> {
    ...
    /// Runs the computation and keeps domain errors separate from interpreter errors.
    ///
    /// The outer Result is AlgebraicError; the inner Result is ErrorEffect<E>.
    fn run_internal<A: 'static>(
        computation: Eff<ErrorEffect<E>, A>,
    ) -> Result<Result<A, E>, AlgebraicError> {
        let normalized = computation.normalize();

        match normalized.inner {
            EffInner::Pure(value) => Ok(Ok(value)),
            EffInner::Impure(operation) => match operation.operation_tag {
                error_operations::THROW => {
                    let error = match operation.arguments.downcast::<E>() {
                        Ok(value) => *value,
                        Err(_) => {
                ...
                _ => Err(AlgebraicError::UnknownOperation {
                    effect: ErrorEffect::<E>::NAME,
...
impl<E: Clone + 'static> Handler<ErrorEffect<E>> for ErrorHandler<E> {
    type Output<A> = Result<Result<A, E>, AlgebraicError>;
...
///     |_| Eff::pure(42)
/// );
/// assert_eq!(handler.run(computation), Ok(42)).unwrap();
/// ```
///
/// ```rust
/// use lambars::effect::algebraic::{ErrorEffect, ErrorHandler, Handler, Eff, catch};
...
///     |_| Eff::pure(0)
/// );
/// assert_eq!(handler.run(computation), Ok(100)).unwrap();
/// ```
    ...
    F: FnOnce(E) -> Eff<ErrorEffect<E>, A> + 'static,
{
    ...
    match ErrorHandler::<E>::run_internal(computation) {
        Ok(Ok(value)) => Eff::pure(value),
        Ok(Err(error)) => recovery(error),
...
{
    match ErrorHandler::<E>::run_internal(computation) {
        Ok(Ok(value)) => Eff::pure(Ok(value)),
        Ok(Err(error)) => Eff::pure(Err(error)),
        Err(error) => Eff::failed(error),
    }
}
...
mod tests {
    ...
    #[rstest]
    fn error_throw_wrong_argument_type_returns_interpreter_error() {
        let malformed =
            Eff::<ErrorEffect<String>, i32>::perform_raw::<i32>(error_operations::THROW, 7_i32);
        assert_eq!(
            ErrorHandler::<String>::new().run(malformed),
    ...
    #[rstest]
    fn error_unknown_operation_returns_interpreter_error() {
        let malformed =
            Eff::<ErrorEffect<String>, i32>::perform_raw::<i32>(OperationTag::new(999), ());
        assert!(matches!(
            ErrorHandler::<String>::new().run(malformed),
    ...
    fn error_fmap_on_success() {
        ...
        let result = handler.run(mapped).unwrap();
        assert_eq!(result, Ok(42));
    }
    ...
    fn error_flat_map_on_success() {
        ...
        let result = handler.run(chained).unwrap();
    ...
    fn error_conditional_throw() {
        ...
        let positive = handler.clone().run(check_positive(10)).unwrap();
        let negative = handler.run(check_positive(-5)).unwrap();
    ...
    fn error_validation_pattern() {
        ...
        let valid_result = handler.clone().run(valid_computation).unwrap();
        assert_eq!(valid_result, Ok(("Alice".to_string(), 30)));

        // Invalid name
        let invalid_name =
            validate_name("").flat_map(|name| validate_age(30).fmap(|age| (name, age)));
        let invalid_name_result = handler.clone().run(invalid_name).unwrap();
        assert_eq!(
            invalid_name_result,
        ...
        let invalid_age_result = handler.run(invalid_age).unwrap();
        assert_eq!(
            invalid_age_result,
