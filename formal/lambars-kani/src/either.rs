...
kani_proof!(either_bimap_on_left_variant_has_documented_left_behavior, {
    let value: i32 = kani::any();
    let mapped =
        Either::<i32, i32>::Left(value).bimap(|x| x.wrapping_add(1), |x| x.wrapping_sub(1));
    assert_eq!(mapped, Either::Left(value.wrapping_add(1)));
});
kani_proof!(
    either_bimap_on_right_variant_has_documented_right_behavior,
    {
        let value: i32 = kani::any();
        let mapped =
            Either::<i32, i32>::Right(value).bimap(|x| x.wrapping_add(1), |x| x.wrapping_sub(1));
...
kani_proof!(
    either_into_options_on_left_variant_has_documented_left_behavior,
    {
        let value: i32 = kani::any();
        assert_eq!(
            Either::<i32, i32>::Left(value).into_options(),
            (Some(value), None)
        );
    }
);
kani_proof!(
    either_into_options_on_right_variant_has_documented_right_behavior,
    {
        let value: i32 = kani::any();
        assert_eq!(
            Either::<i32, i32>::Right(value).into_options(),
            (None, Some(value))
        );
    }
);
...
kani_proof!(either_bimap_matches_map_left_followed_by_map_right, {
    ...
    let expected = either
        .clone()
        .map_left(|x| x.wrapping_add(1))
        .map_right(|x| x.wrapping_sub(1));
