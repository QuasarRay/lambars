...
kani_proof!(metaverification_boundary_triplet_adapter_matches_handwritten_reference_shape, {
    let boundary: usize = kani::any();
    kani::assume(boundary > 0 && boundary < usize::MAX);
    assert_eq!(boundary_points(boundary), handwritten_boundary_points(boundary));
});

kani_proof!(metaverification_generated_kani_boundary_adapter_matches_shared_adapter, {
    let boundary: usize = kani::any();
    kani::assume(boundary > 0 && boundary < usize::MAX);
    assert_eq!(boundary_points__kani_adapter(boundary), boundary_points(boundary));
});

kani_proof!(metaverification_generated_verus_boundary_adapter_matches_shared_adapter, {
    let boundary: usize = kani::any();
    kani::assume(boundary > 0 && boundary < usize::MAX);
    assert_eq!(boundary_points__verus_adapter(boundary), boundary_points(boundary));
});

kani_proof!(metaverification_delegate_macro_boundary_function_matches_shared_target, {
    let boundary: usize = kani::any();
    kani::assume(boundary > 0 && boundary < usize::MAX);
    assert_eq!(delegated_boundary_points(boundary), boundary_points(boundary));
});

kani_proof!(metaverification_law_triplet_adapter_is_exact_boolean_conjunction, {
...
kani_proof!(metaverification_delegate_macro_law_triplet_matches_shared_target, {
    let left_identity: bool = kani::any();
    let right_identity: bool = kani::any();
    let associativity: bool = kani::any();
    assert_eq!(
